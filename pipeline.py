...
# File Paths
TARGET_CHECKPOINT_FILE = "best.onnx"
...
class TargetShapeText:
    ...
    def __init__(self):
        ...
        # initialize model
        self.session = ort.InferenceSession(TARGET_CHECKPOINT_FILE, 
    ...
    def __runText(self, img):
        ...
        for i,box in enumerate(self.boxes):
            frame = img[box[1]+CROP_AMNT:box[3]-CROP_AMNT,box[0]+CROP_AMNT:box[2]-CROP_AMNT]
            frame = cv2.resize(frame, (120,120))
            gray = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
            row = int(i / 4) * 120
            col = int(i % 4) * 120
            input_img[row:row+120, col:col+120] = gray
        cv2.imwrite("text.png",input_img)
    ...
    def run(self, img, text=False):
        ...
        for row in range(0, img.shape[0] - FRAME_SIZE, STEP):
            for col in range(0,img.shape[1] - FRAME_SIZE, STEP):
                ...
                threads[count] = Thread(target = run_model, args=(input_tensor, self.boxes, count, row, col))
                threads[count].start()
                count += 1
